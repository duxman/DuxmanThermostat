...
class StopableProduceThread(threading.Thread):
    ConsumerQueue = None
    Logger = None
    def __init__(self, queue = None,target = None, sleep = 0.1, name = "DefaultConsumerThread", logger = None):
        """
    ...
    def run(self):
        ...
        while self.isRunning() and self.Target != None:
            if  self.ConsumerQueue !=None:
                """"Obtenemos el valor y lo introducimos en la cola"""
                valor = self.Target()
                self.ConsumerQueue.put( valor )
        ...
        """Fin del proceso"""
        self.Logger.info( "Exit Threat " + self.name )
